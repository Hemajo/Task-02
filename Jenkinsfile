  ...
  stages{
	...
	stage("Upload to Nexus"){
	  ...
	  stage('Pull the Artifact from Nexus and Deploy on Production') {
		  ...
		  steps {
			  sh 'wget --user=admin --password=admin http://65.1.110.124:8081/repository/Jenkins_Assignment_3/com/blazeclan/DevOpsDemo/1.2/DevOpsDemo-1.2.war'			  deploy adapters: [
